...
# correlation between mean difference and mean confidence
# Rouault 
rouault_mean_diff_confidence_cor <- cor.test(Rouault2018_df$mean_diff, Rouault2018_df$mean_confidence, method = "pearson")

# Seow & Gillan
seowgillan_mean_diff_confidence_cor <-cor.test(SeowGillan2020_df$mean_diff, SeowGillan2020_df$mean_confidence, method = "pearson")

# Hoven
hoven_mean_diff_confidence_cor <-cor.test(hoven_semantic_space$mean_diff, hoven_semantic_space$mean_confidence, method = "pearson")
...
mean_rating_rouault <- mean(Rouault2018_df$mean_diff)
median_rating_rouault <- median(Rouault2018_df$mean_diff)
mean_confidence_rouault <- mean(Rouault2018_df$mean_confidence)
median_confidence_rouault <- median(Rouault2018_df$mean_confidence)

# Calculate mean and median for Seow & Gillan 2020 
mean_rating_seowgillan <- mean(SeowGillan2020_df$mean_diff)
median_rating_seowgillan <- median(SeowGillan2020_df$mean_diff)
mean_confidence_seowgillan <- mean(SeowGillan2020_df$mean_confidence)
median_confidence_seowgillan <- median(SeowGillan2020_df$mean_confidence)

# Calculate mean and median for Hoven et al 2023
mean_rating_hoven <- mean(hoven_semantic_space$mean_diff)
median_rating_hoven <- median(hoven_semantic_space$mean_diff)
mean_confidence_hoven <- mean(hoven_semantic_space$mean_confidence)
median_confidence_hoven <- median(hoven_semantic_space$mean_confidence)
...
rouault_mean_rating_fig <- 
  ...
  ggpubr::stat_cor(method = "pearson", color = "black", show.legend = FALSE,
                   label.y = 1, label.x = -2.3, size = 8,
                   aes(label = paste0("italic(r) == ", ..r.., "*','~italic(p) == ", ..p..))) +  
  labs(title = "Rouault et al., 2018", y= "Mean confidence rating (1-6 scale)", x= "Rating delta") +
  theme_minimal() +
  theme(
  ...
  coord_cartesian(xlim = c(-2.5, 1.5), ylim = c(1,6)) +
...
seowGillan_mean_rating_fig <- 
  ...
  geom_smooth(method = "lm", se = TRUE, color = "red", show.legend = FALSE) +
  ggpubr::stat_cor(method = "pearson", color = "black", show.legend = FALSE,
                   label.y = 0, label.x = -2.3, size = 8,
                   aes(label = paste0("italic(r) == ", ..r.., "*','~italic(p) == ", ..p..))) +  
  labs(title = "Seow & Gillan,  2020", y= "Mean confidence rating (1-100 scale)", x= "Rating delta") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 24),
    axis.title = element_text(size = 24),
  ...
  coord_cartesian(xlim = c(-2.5, 1.5), ylim = c(1,100)) +
...
hoven_mean_rating_fig  <- 
  ...
  geom_smooth(method = "lm", se = TRUE, color = "red", show.legend = FALSE) +
  ggpubr::stat_cor(method = "pearson", color = "black", show.legend = FALSE,
                   label.y = 50, label.x = -2.3, size = 8,
                   aes(label = paste0("italic(r) == ", ..r.., "*','~italic(p) == ", ..p..))) +
  labs(title = "Hoven et al., 2023", y= "Mean confidence rating (1-6 scale)", x= "Rating delta") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 24),
    axis.title = element_text(size = 24),
  ...
  coord_cartesian(xlim = c(-2.5, 1.5), ylim = c(50,100)) +
  scale_y_continuous(sec.axis = dup_axis(name = "Mean confidence rating (50-100 scale)"))

# Marginal densities for Rouault
xdens_rouault <- cowplot::axis_canvas(rouault_mean_rating_fig, axis = "x") +
  geom_density(data = Rouault2018_df, aes(x = mean_diff),
...
ydens_rouault <- cowplot::axis_canvas(rouault_mean_rating_fig, axis = "y", coord_flip = TRUE) +
  geom_density(data = Rouault2018_df, aes(x = mean_confidence),
...
p_with_marginal_rouault <- insert_xaxis_grob(rouault_mean_rating_fig, xdens_rouault, grid::unit(.2, "null"), position = "top")
...
xdens_seowgillan <- axis_canvas(seowGillan_mean_rating_fig, axis = "x") +
  geom_density(data = SeowGillan2020_df, aes(x = mean_diff),
...
ydens_seowgillan <- axis_canvas(seowGillan_mean_rating_fig, axis = "y", coord_flip = TRUE) +
  geom_density(data = SeowGillan2020_df, aes(x = mean_confidence),
               alpha = 0.7, fill = colors[2], color = "black", adjust = 1) +
...
p_with_marginal_seowgillan <- insert_xaxis_grob(seowGillan_mean_rating_fig, xdens_seowgillan, grid::unit(.2, "null"), position = "top")
...
xdens_hoven <- axis_canvas(hoven_mean_rating_fig, axis = "x") +
  geom_density(data = hoven_semantic_space, aes(x = mean_diff),
               alpha = 0.7, fill = colors[3], color = "black", adjust = 1) +
...
ydens_hoven <- axis_canvas(hoven_mean_rating_fig, axis = "y", coord_flip = TRUE) +
  geom_density(data = hoven_semantic_space, aes(x = mean_confidence),
               alpha = 0.7, fill = colors[2], color = "black", adjust = 1) +
...
p_with_marginal_hoven <- insert_xaxis_grob(hoven_mean_rating_fig, xdens_hoven, grid::unit(.2, "null"), position = "top")
...
R2018_reversed_fig <-
  ...
  geom_line(data = . %>% filter(item == "STAI.2" | item == "STAI.1"),
            aes(group = 1), color = "black", size = 1, alpha=0.6) +
...
hoven_reversed_fig <-
  ...
  geom_signif(comparisons = list(c("not_reversed", "reversed")),
              ...
              textsize = 7,
              y_position = 0.25) +
  labs(title = "Hoven et al., 2023", x = "", y = "") +
  scale_shape_manual(
    values = questionnaire_shape_map,
    labels = c(
      "depression" = "Depression (SDS)",
...
```{r reversed-items, echo=FALSE, fig.cap="Relationship Between Reversed-Coded Items and Item Confidence Correlation. Each point represents a questionnaire item with the shapes indicating different questionnaires. Standard and reversed items are color-coded. Questionnaires: Anxiety - State-Trait Anxiety Inventory (STAI; Spielberger, 1970). Apathy - Apathy Evaluation Scale (AES; Marin et al., 1991). Depression - Self-Rating Depression Scale (SDS; Zung, 1965). Impulsivity - Barratt Impulsivity Scale (BIS-11; Patton et al., 1995). Reference line at y=0 indicates zero item confidence correlation. Left panel: results from Rouault et al., 2018, highlighting items STAI.2 (standard; ‘I feel nervous and restless’) and STAI.1 (reversed; ‘I feel pleasant’). Middle panel: results from Seow & Gillan, 2020, highlighting items SDS.1 (standard; ‘I feel down-hearted and blue’) and SDS.17 (reversed; ‘I feel that I am useful and needed’). Right panel: results from Hovel et al. 2023, highlighting items BIS.4 (standard; ‘I am happy-go-lucky’) and BIS.8 (reversed; ‘I am self-controlled.’). STAI items are not shown in the Hoven et al. 2023 panel because a different anxiety inventory was used in their study, which did not include reversed items.  Asterisks denote significance (p<0.001) of a t-test for independent samples."}
...
combined_skewness_conf_fig <- skew_conf_top_panel / skew_conf_bottom_panel + plot_layout(heights = c(2, 1.5))
...
R2018_column <- wrap_plots(
  R2018_CIT_main_fig,
  R2018_AD_main_fig,
  ncol = 1, nrow = 2
)

SG2020_column <- wrap_plots(
  SG_CIT_main_fig, SG_CIT_mean_fig,
  SG_AD_main_fig, SG_AD_mean_fig,
  ncol = 2, nrow = 2,
  widths = c(6, 1)
)

dimensions_combined_plot <- wrap_plots(
  R2018_column, SG2020_column,
  ncol = 2
) + 
...
ggsave(filename = "../figures/dimensions_combined_plot.png", dimensions_combined_plot, width = 14, height = 10, dpi = 300)
...
# Hoven et al. (2023) — random slope + intercept by questionnaire
...
[^1]: As Hoven et al., 2023 used a questionnaire set that did not match the original set of Gillan et al. (2016; included the GAD-7 instead of the STAI to measure anxiety) we report its analysis separately in the Appendix
...
knitr::include_graphics("../figures/dimensions_combined_plot.png")
```
...
# OCI SDS correlation without inattentive participants
oci_sds_cor <- mean_conf_df %>%
  filter(failed_attention == "Attentive") %>%
  cor.test(~ total_oci + total_sds, data = ., method = "pearson")

# acquiescence with inattentive responding
mean_conf_df %>% 
...
# inattentive mean rating t.test
inattentive_rating_ttest <- 
  mean_conf_df %>% 
  mutate(failed_attention = factor(failed_attention, 
                                   levels = c("Inattentive", "Attentive"))) %>% 
  t.test(mean_rating ~ failed_attention, paired =FALSE, data = .)

# cohen's D
attentiveness_acq_d <- cohens_d(
  mean_rating ~ failed_attention,
  data = mean_conf_df %>% 
    mutate(failed_attention = factor(failed_attention, 
                                    levels = c("Inattentive", "Attentive")))
)

# plot total oci corrected score with total oci 
oci_oci_corrected_plot <- 
mean_conf_df %>%
  filter(failed_attention == "Attentive") %>%
  ggplot(aes(x = total_oci, y = total_oci_corrected_scaled)) +
  geom_point(alpha=0.5, size=2) +
  stat_smooth(method = "lm", se = FALSE) +
  ggpubr::stat_cor(method = "pearson", size=5) +
  labs(x = "Total OCI-R score", y = "Total OCI-R corrected score", color = "Attention Status") +
  theme_minimal() +
  theme(text = element_text(size = 16))

sds_sds_corrected_plot <- 
mean_conf_df %>%
  filter(failed_attention == "Attentive") %>%
  ggplot(aes(x = total_sds, y = total_sds_corrected_scaled)) +
  geom_point(alpha=0.5, size=2) +
  stat_smooth(method = "lm", se = FALSE) +
  ggpubr::stat_cor(method = "pearson", size=5) +
  labs(x = "Total SDS score", y = "Total SDS corrected score", color = "Attention Status") +
  theme_minimal() +
  theme(text = element_text(size = 16))
...
[^2]: A common practice is to include both anxiety and OCD measures in one multiple regression analysis to reveal their unique contributions. However, when predictors are highly correlated (as was the case between OCI-R and SDS: `r apa_print(oci_sds_cor)$estimate`), regression coefficients become difficult to interpret as revealing true associations with the predicted variable (here, mean confidence), especially when coefficients show opposite signs (see @miller2001).
