...
We elaborate on these two independent factors in the Method section and demonstrate their respective contributions to the reported associations between mental health and metacognition in three large datasets [@rouault2018; @seow2020; @hoven2023a] in the Results section. Finally, analysis of a new dataset with direct measures of inattentive responding and acquiescence reveals that variability in these surface-level properties of questionnaire-filling behaviour may explain a puzzling finding, obtained only in online studies: over-confidence, rather than the well- documented under-confidence, among participants with compulsive or obsessive-compulsive tendencies.
...
hoven_native_space <- hoven_semantic_space %>% 
 select(id, mean_confidence, all_of(all_items_hoven))
...
hoven_native_space[hoven_reversed_items] <- lapply(hoven_native_space[hoven_reversed_items], reverse_scores)
...
hoven_native_space[all_items_hoven] <- lapply(hoven_native_space[all_items_hoven], normalize_scores)
...
hoven_native_space <- hoven_native_space %>% 
  rowwise() %>% 
  mutate(mean_rating_score = mean(c_across(all_of(setdiff(all_items_hoven, c("mean_confidence", "eat.25")))), na.rm = TRUE)) %>%
  ungroup()
...
# Calculate mean and median for Hoven et al 2023
mean_rating_hoven <- mean(hoven_native_space$mean_rating_score)
median_rating_hoven <- median(hoven_native_space$mean_rating_score)
mean_confidence_hoven <- mean(hoven_native_space$mean_confidence)
median_confidence_hoven <- median(hoven_native_space$mean_confidence)
...
# Response bias plot for Hoven et al 2023 
hoven_native_space_fig  <-
  ggplot(hoven_native_space, aes(x=mean_rating_score, y=mean_confidence)) +
  geom_point(alpha=0.7, size=2.5) +
  geom_smooth(method = "lm", se = TRUE, color = "red", show.legend = FALSE) +
  ggpubr::stat_cor(method = "pearson", color = "black", show.legend = FALSE, 
                   label.y = 50, label.x = 0.2, size = 8,
                   aes(label = paste0("italic(r) == ", ..r.., "*','~italic(p) == ", ..p..))) +
  labs(title = "Hoven et al., 2023", y= "Mean confidence rating (1-6 scale)", x= "Mean item rating") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 24),
    axis.title = element_text(size = 24),
  ...
  coord_cartesian(xlim=c(0.18, 0.8), ylim = c(50,100)) +
  scale_y_continuous(sec.axis = dup_axis(name = "Mean confidence rating (50-100 scale)"))
...
# Marginal densities for Hoven
xdens_hoven <- axis_canvas(hoven_native_space_fig, axis = "x") +
  geom_density(data = hoven_native_space, aes(x = mean_rating_score),
               alpha = 0.7, fill = colors[3], color = "black", adjust = 1) +
  geom_vline(xintercept = mean_rating_hoven, color = "red", linetype = "dashed") +
  geom_vline(xintercept = median_rating_hoven, color = "blue", linetype = "dashed")

ydens_hoven <- axis_canvas(hoven_native_space_fig, axis = "y", coord_flip = TRUE) +
  geom_density(data = hoven_native_space, aes(x = mean_confidence),
               alpha = 0.7, fill = colors[2], color = "black", adjust = 1) +
  geom_vline(xintercept = mean_confidence_hoven, color = "red", linetype = "dashed") +
  geom_vline(xintercept = median_confidence_hoven, color = "blue", linetype = "dashed") +
  coord_flip() +
  scale_y_reverse()

p_with_marginal_hoven <- insert_xaxis_grob(hoven_native_space_fig, xdens_hoven, grid::unit(.2, "null"), position = "top")
p_with_marginal_hoven <- insert_yaxis_grob(p_with_marginal_hoven, ydens_hoven, grid::unit(.2, "null"), position = "left")

# Combine the plots into one figure
mean_rating_figure <- ggdraw(p_with_marginal_rouault) | 
  ggdraw(p_with_marginal_seowgillan) | 
  ggdraw(p_with_marginal_hoven)
...
hoven_rating_bias_cor <- 
  cor.test(hoven_native_space$mean_rating_score, hoven_native_space$mean_confidence, 
           method = 'pearson')
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#----- perceptual discrimination task Hoven et al 2023 -------
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# List of questionnaires
...
hoven_reversed_fig <- 
  ...
  stat_summary(fun.data = mean_se,
               ...
               color = "black",
               group = 1) +
  geom_line(data = . %>% filter(item %in% c("bis.4", "bis.8")), 
            aes(group = 1), color = impulsivity_color, size = 1, alpha = 0.6) +
  ...
  geom_signif(comparisons = list(c("not_reversed", "reversed")), 
              ...
              textsize = 7,
              y_position = 0.25) +
  labs(title = "Hoven et al., 2023", x = "", y = "", color = "Questionnaire") + 
  scale_color_manual(values = c(
    anxiety = anxiety_color,
    apathy = apathy_color,
    depression = depression_color,
    impulsivity = impulsivity_color
  )) +
  scale_x_discrete(labels = c("not_reversed" = "Standard", "reversed" = "Reversed")) +
  coord_cartesian(ylim = c(-0.2, 0.3)) +
  theme_bw() +
...
reversed_items_fig <- R2018_reversed_fig + 
  SG2020_reversed_fig + 
  hoven_reversed_fig + theme(legend.position = "none") +
...
# create the r and p value label for Hoven et al., 2023
...
hoven_items_table %>%
  ...
  #ggrepel::geom_text_repel(aes(label = item), size = 3.5, max.overlaps = Inf, segment.size = 0.3) +
  geom_point(alpha = 0.7, size=2.5) +
  labs(subtitle = "Hoven et al., 2023",
       x = "Skewness",
       y = "Item Confidence Correlation",
       shape = "Questionnaire",
       color= "Reversed") +  
...
plot_SG_zung2 <- 
 ggplot(SeowGillan2020_df, aes(x = factor(zung.2), y = mean_confidence)) +
  geom_jitter(width = 0.1, height = 0.0, alpha = 0.5, shape=17, color="#D95F02" ) +
  stat_summary(fun = mean,
               geom = "point",
               position = position_dodge(0.9),
               size = 2,
...
top_panel <- Rouault2018_skewness_conf|SeowGillan_skewness_conf|hoven_skew_conf
bottom_panel <- plot_Rouault_eat_2|plot_SG_zung2
...
# ==========================================================
# MLM with random intercept for questionnaire 
# ==========================================================
...
hoven_CIT_dimensions_fig <- 
  ...
  coord_cartesian(x=c(0,6))+
  labs(x = 'Item-skewness', y = 'Item-weight', title = 'Hoven et al., 2023: CIT') +  
  theme(
    legend.position = "none",
    plot.title = element_text(size = 20),
    axis.title = element_text(size = 18),
...
hoven_AD_dimensions_fig <- 
  ...
  scale_color_manual_values +
  theme_minimal() +
  labs(x = 'Item-skewness', title = 'Hoven et al., 2023: AD') +  
  coord_cartesian(xlim=c(0,6))+
  theme(legend.position = "none", axis.title.y = element_blank(),
    plot.title = element_text(size = 20),
    axis.title = element_text(size = 18),
...
H2023_count_AD_reversed <- hoven_items_table_no_anx %>%
  arrange(desc(AD)) %>%
  slice_head(n = 15) %>% 
  count(is_reversed)
...
# add failed attention and accuracy 
...
oci_processing <- self_long %>%
  ...
  group_by(item_name) %>% 
  mutate(
    item_corrected = case_when(
      failed_attention == "Attentive" ~ {
        # Fit model using only attentive participants for this item
        attentive_data <- cur_data() %>% filter(failed_attention == "Attentive")
        model <- lm(item_value ~ mean_rating, data = attentive_data)
        
        # Get residuals and map them back
        residuals_vec <- as.numeric(residuals(model))
        residuals_vec[match(subj_id, attentive_data$subj_id)]
      },
      TRUE ~ NA_real_
    )
  ) %>%
...
# Calculate both original and corrected totals
oci_totals <- oci_processing %>%
  group_by(subj_id) %>%
  summarise(
    total_oci = sum(item_value, na.rm = TRUE), # Original total OCI score
    total_oci_corrected = sum(item_corrected, na.rm = TRUE), # Corrected total OCI score
...
# SDS processing (following OCI pattern)
sds_processing <- self_long %>%
  filter(str_detect(item_name, "^zung_")) %>% # find all SDS items 
  filter(item_name != 'zung_inf_1' & item_name != 'zung_inf_2') %>% # remove inf items 
  mutate(item_value = as.numeric(item_value)) %>% # convert item_value to numeric
  ...
  left_join(mean_conf_df %>% select(subj_id, mean_rating, failed_attention), by = "subj_id") %>% # add mean rating and attention status
  group_by(item_name) %>% # for each item 
  mutate(
    item_corrected = case_when(
      failed_attention == "Attentive" ~ {
        # Fit model using only attentive participants for this item
...
sds_totals <- sds_processing %>%
  ...
  summarise(
    ...
    total_sds_corrected = sum(item_corrected, na.rm = TRUE), # Corrected total SDS score
    failed_attention = first(failed_attention), # Keep attention status
    .groups = "drop"
  ) %>%
  mutate(
...
#ggsave("../figures/inattentive_acq_fig.png", inattentive_acq_fig, width = 12,height = 12, dpi = 300)       
