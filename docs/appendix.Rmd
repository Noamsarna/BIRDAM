...
### Mixed linear model analysis of skewness effects on item-confidence correlation
To examine whether the relationship between item skewness and item-confidence correlation was consistent across different questionnaires, we fitted a linear mixed models predicting item-confidence correlation from item-level skewness, including random intercepts and random slopes for questionnaire (conf_cor ~ skewness + (1 + skewness | questionnaire)). Results revealed a significant positive effect of skewness in all three datasets, even when allowing the strength of the relationship to vary across questionnaires: Rouault et al., 2018: (`r apa_print(rouault_skew_conf_lmm)$full_result$skewness`), Seow & Gillan, 2020: (`r apa_print(sg_skew_conf_lmm)$full_result$skewness`) and Hoven et al., 2023 (`r apa_print(hoven_skew_conf_lmm)$full_result$skewness`).The Hoven model showed signs of overfitting, likely due to the smaller number of anxiety items (7 items in the GAD-7 vs. 20 items in the STAI used in the Rouault and Seow & Gillan datasets). Therefore, we fitted an additional model with random intercept only (conf_cor ~ skewness + (1 | questionnaire)). This model also revealed a significant positive effect of skewness on item–confidence correlation (`r apa_print(hoven_skew_conf_lmm_intercept_only)$full_result$skewness`), supporting a general within-questionnaire relationship that is not solely attributable to between-questionnaire differences.


```{r within-quest-analysis-R2018, include = TRUE, echo=FALSE, fig.cap="Figure A1 Within-questionnaire relationship of item kkewness and item-confidence correlations in Rouault et al. (2018). Each panel represents a different questionnaire, and each point represents an individual item. The x-axi3s shows the skewness of item responses. Y-axis shows the correlation between item ratings and confidence across participants. The black line represents the linear fit with 95% confidence intervals (gray shading). Spearman correlation coefficients (R) and corresponding p-values are shown for each questionnaire.", fig.lp=""}

knitr::include_graphics("../figures/skewness_confidence_by_questionnaire_R2018.png") 
```

```{r figure_within-quest-analysis-SG2020, include = TRUE, echo=FALSE, fig.cap="Figure A2 Within-questionnaire relationship of item kkewness and item-confidence correlations in Seow and Gillan (2020). The same conventions as in Figure A1 are used."}
knitr::include_graphics("../figures/skewness_confidence_by_questionnaire_SG2020.png")
```

```{r figure-within-quest-analysis-H2023, include = TRUE, echo=FALSE, fig.cap="Figure A3 Within-questionnaire relationship of item kkewness and item-confidence correlations in Hoven et al. (2023). The same conventions as in Figure A1 are used."}
knitr::include_graphics("../figures/skewness_confidence_by_questionnaire_H2023.png")
```
...
ggsave(
  filename = "../figures/figure_R2018_unfiltered.png",
  plot = figure_R2018_unfiltered,
  width = 10,
  height = 6,
  dpi = 300
)
...
```{r figure_R2018_unfiltered_fig, fig.cap="Figure A4 Correlation between Item -Level Skewness and Confidence with Extreme Skewed Items Included. Relationship between item-level skewness and item-confidence correlation in Rouault et al., (2018) including full skewness scale. Each point represents an item from the self-report questionnaires, with the shapes indicating different questionnaires and color indicating whether the item was reversed or not. The x-axis represents the skewness score of each item; the y-axis represents the Spearman correlation coefficient between the item’s skewness and mean confidence ratings."}
knitr::include_graphics("../figures/figure_R2018_unfiltered.png")
```



### Associations between transdiagnostic dimensional weights with skewness and coding direction in Hoven et al., 2023 

As Hoven et al., 2023 used the GAD-7 instead of the STAI to measure anxiety (as was used in the original factor analysis performed by Gillan et al., 2016), we extracted factor weights only for the items that were common across the two studies. Which were all the other measures except the GAD-7. Consistent with Analysis 3 in both Rouault et al. (2018) and Seow and Gillan (2020), our reanalysis of Hoven et al. (2023) revealed a positive correlation between item weight and item skewness on the CIT dimension, such that more skewed items contributed more strongly to the CIT factor (`r apa_print(H2023_item_weight_skewness_cor_CIT)$full_result`; Figure A5, left panel). Similarly, the AD dimension showed a pattern consistent with the other two datasets, with reversed items of low skewness loading heavily on the AD factor (Figure A5, right panel).

```{r figure_H2023_item_weight_skewness, include = TRUE, echo=FALSE, fig.cap="Figure A5 The effect of reversed items and skewness on the CIT and AD dimensions in Hoven et al. (2023). Same conventions as Figure 6 are used."}
...
```{r sex_differences_inatten, echo=FALSE, fig.cap="Figure A6 – Sex differences in inattentiveness prevalence. "}
knitr::include_graphics("../figures/sex_differences_inatten_fig.png")
```

```{r sex_differences_conf, echo=FALSE, fig.cap="Figure A7 – Mean Confidence by Attentiveness and Sex. Scatter plot showing mean confidence ratings across attentive and inattentive responders, separated by sex. Blue and pink points represent attentive and inattentive participants, respectively. Error bars indicate the standard error for each subgroup."}
...
Higher increments result in easier perceptual decisions. The mean difficulty was then calculated for each participant to assess overall task difficulty. We found that inattentive participants encountered significantly easier task on average compared to attentive participants, `r apa_print(difficulty_t_test)$statistic` (figure A8 left panel). We also found a negative correlation between task difficulty and mean confidence ratings, indicating that as the task became easier, mean confidence increased, `r apa_print(difficulty_confidence_cor)$statistic` (figure A8, right panel).

```{r staircase_figure, echo=FALSE, fig.cap="Figure A8 – Task difficulty and inattentiveness. Left panel: mean difficulty ratings as a function of attentiveness, with data points for attentive and inattentive responses shown in red and blue, respectively. Black markers and error bars represent the mean and standard error, respectively. Right panel: mean confidence as a function of mean difficulty, with attentive and inattentive responses again differentiated by color. A negative correlation is observed, as shown by the black regression line (shaded area represents the confidence interval)."}
